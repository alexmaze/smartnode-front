<template>
  <div class="node node-device node-device-sensor node-device-sensor-infrared" :id="data.id">
    ...
    <div class="body">
      <div class="title" :id="data.id + '-title'">{{data.label}}</div>
    </div>
  </div>
</template>
...
export default {
  mixins: [Node],
  name: 'device-sensor-infrared-node',
  components: {
  },
  data () {
    return {
  ...
  methods: {
    init () {
      ...
      let ret = this.instance.addEndpoint(this.data.id, {
        uuid: this.data.id + '-output-0',
        anchor: [0.83, 0.935, 0, 0],
